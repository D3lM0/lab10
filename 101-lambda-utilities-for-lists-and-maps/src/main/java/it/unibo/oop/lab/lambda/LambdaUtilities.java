...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        ...
        list.forEach(elem -> {
            Optional<T> elemToAdd = Optional.of(elem).filter(pre);
            filteredList.add(elemToAdd);
        });
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(elem -> {
            final var key = op.apply(elem);
            map.merge(key, new HashSet<>(Set.of(elem)), (oldSet, newSet) -> {
                oldSet.add(elem);
                return oldSet;
            });
        });
